...
<div class="opacity-80 font-serif text-sm" hx-get="/you-pentacles.html" hx-target="#content">You will spend 1 pentacle</div>
          ...
          <button id="set_intention" type="button" class="transition-opacity origin-top duration-1000 bg-brand text-lg font-serif text-white rounded-xl px-6 py-3">Save Intention</button>
        </form>
      </div>
    `;
    ...
    document.getElementById('set_intention').addEventListener('click', async () => {
      if (this.pentacles < 1) {
        if (confirm('You need 1 more pentacle to set an intention. Click OK to purchase more.')) {
