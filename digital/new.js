...
class DigitalNew extends HTMLElement {
  constructor() {
    super();
    this.index = 0;
  }
  ...
  render() {
    this.innerHTML = `
    <title-bar data-back-link="/digital-index.html" class="w-full" title="Digital Tarot"></title-bar>
    <form method="post" enctype="multipart/form-data" id="new" class="w-full mx-auto flex-col px-6 flex-1 flex items-center justify-start gap-6">
      <div class="field flex flex-col items-center justify-between p-4 bg-translucent gap-3 w-full rounded-2xl">
        <label for="digital-title" class="label opacity-80 font-serif">Name</label>
        <textarea placeholder="Type here" id="digital-title" name="title" class="w-full text-center bg-transparent focus:text-black focus:bg-white px-6 py-2 rounded-lg"></textarea>
...
<div class="opacity-80 font-serif text-sm" hx-get="/you-pentacles.html" hx-target="#content">Get 10 ideas = 1 pentacle</div>
    ...
    document.getElementById('continue').addEventListener('click', async (event) => {
      ...
      formData.append('title', document.getElementById('digital-title').value);
      formData.append('id', '');
      ...
      try {
        ...
        if (response.ok) {
          ...
          await Preferences.set({ key: 'digital-slug', value: data });
          const getBoardSlug = await Preferences.get({ key: 'digital-slug' });

          if (getBoardSlug.value) {
            htmx.ajax('GET', '/digital-entry.html', { target: '#content' });
          } else {
            console.error('Failed to set digital-slug');
          }
        } else {
...
customElements.define('digital-new', DigitalNew);
