  ...
  render() {
    const entriesHtml = this.entries.map(entry => `
      <a href="/app/vision-boards/entry.html?id=${entry.slug}" class="flex flex-col items-center">
        <div class="text-2xl w-[72px] h-[72px] lg:w-[96px] lg:h-[96px] flex-shrink-0 flex items-center justify-center rounded-xl bg-neutral text-black bg-cover bg-no-repeat bg-center" style="background: ${entry.icon.includes('//') ? `url(${entry.icon})` : `${entry.icon}`}">
    ...
    `).join('');
    this.innerHTML = `
    <title-bar root="true" class="w-full" title="Vision Boards"></title-bar>
